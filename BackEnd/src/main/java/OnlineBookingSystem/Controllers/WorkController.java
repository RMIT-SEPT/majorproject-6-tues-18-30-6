...
import OnlineBookingSystem.SystemException;
...
public class WorkController {
    ...
    public ModelAndView viewBookingsForWork(@PathVariable(name="workid") int workId){
        ...
        try{
            ...
            Work work = onlineBooking.getWorkById(workId);
            if(work == null){
                throw new SystemException("Invaid Work ID");
            }         
            ArrayList<Booking> bookings = onlineBooking.getBookingsForEmployee(work.getEmployee().getId());
            // Prepare the table
            ...
            return mav;
        }catch(SystemException onlineBookinge){
            logger.info(onlineBookinge.getMessage());
