...
export default class AdminManageTimes extends Component  {
  constructor(props) {
   ...
   this.today = new Date();
   this.firstDay = new Date(this.today.getFullYear(), this.today.getMonth(),1).getDate();
...
minuteChange = (e) =>{
 ...
 for(let i = parseInt(this.firstDay); i <= parseInt(this.lastDay) ; i++ ){
   listOfDates.push(<tr><td onClick={()=>this.toggleDetail(i)}>{i}</td></tr>)
 }
 ...
 if(this.state.showDetail){
   ...
   var content = this.state.times.get(this.clickedDay).map((item,index)=>
   <div>{item} <button id ={index} onClick={this.deleteTime}>delete</button></div>
   );
 ...
 return(
  <div>
    ...
    <h1>{this.monthNames[this.today.getMonth()]} {this.clickedDay}</h1>
